...
export type SessionTokenRow = { modelID: string; input: number; cachedInput: number; totalInput: number; output: number };
export type SessionTokenSummary = { sessionID: string; models: SessionTokenRow[]; totalInput: number; totalCachedInput: number; totalCombinedInput: number; totalOutput: number };

export async function getSessionTokenSummary(sessionID: string): Promise<SessionTokenSummary | null> {
  ...
  let totalInput = 0, totalCachedInput = 0, totalCombinedInput = 0, totalOutput = 0;
  ...
  for (const msg of sessionMessages) {
    ...
    if (existing) { existing.input += input; existing.cachedInput += cachedInput; existing.totalInput += totalInputForMessage; existing.output += output; }
    else { byModel.set(modelID, { input, cachedInput, totalInput: totalInputForMessage, output }); }
  }

  const models = Array.from(byModel.entries())
    .map(([modelID, t]) => ({ modelID, input: t.input, cachedInput: t.cachedInput, totalInput: t.totalInput, output: t.output }))
    .filter((m) => m.totalInput > 0 || m.output > 0)
    .sort((a, b) => b.totalInput + b.output - (a.totalInput + a.output));

  return { sessionID, models, totalInput, totalCachedInput, totalCombinedInput, totalOutput };
}
