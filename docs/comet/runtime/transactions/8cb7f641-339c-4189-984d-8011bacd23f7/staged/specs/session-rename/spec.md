...
an invalid title — in which case it posts a specific error and lets the LLM
react.
...
`session.prompt` **without** `noReply` (so the LLM generates a reply) and
**without** `ignored` on the text part (so the error text reaches the LLM). The
error text names the specific constraint violated, quoting the supplied input:
...
- **Disallowed characters** — the title has 3-8 whitespace-separated tokens but
  ...
  `The session name "<input>" is invalid: it may contain only letters, numbers, apostrophes, and hyphens (3 to 8 words).`

The hook awaits the LLM reply (the prompt resolves only after the reply
completes), then aborts the command. This is the only path that posts a chat
message.
...
letter or number. The rules are unchanged from the prior behavior; only the
**reported reason** for rejection is new.
