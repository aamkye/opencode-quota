...
concept_id: tests/ses-tokens-mounted/resolveList
language: typescript
---
...
```typescript
resolveList(result: ClientResult<readonly unknown[]> = { data: [{ id: options.sessionID ?? "session-a" }] })
```

## Source
Lines 379–384 in `tests/ses-tokens-mounted.fixture.ts`
