...
- [x] 2.9 React to synchronized same-session user-message model changes through the supported TUI event bus even when the host message accessor itself is not reactive, while preserving one refresh per adapter change and lifecycle cleanup.
