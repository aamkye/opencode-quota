...
The system SHALL define quota, home, token-report, and MCP in one data-only manifest that records each source entry, output artifact, normalized runtime ID, optional slot order, and option ownership.
...
- **WHEN** the four current plugins load
- **THEN** their IDs use `aamkye/opencode-tools-quota`, `aamkye/opencode-tools-home`, `aamkye/opencode-tools-token-report`, and `aamkye/opencode-tools-mcp`
...
- **WHEN** quota, home, token-report, and MCP entries are reviewed
...
The build SHALL emit independent quota, home, token-report, and MCP artifacts that import the managed shared artifact and activate no sibling feature.

#### Scenario: Production build completes
- **WHEN** the plugin build runs
- **THEN** it emits all manifest artifacts without generating a composed quota, home, and token-report activation entry
...
Deployment SHALL replace the legacy composed managed entry with manifest-ordered standalone entries while preserving unrelated configuration and applying legacy options only to quota.

#### Scenario: Legacy configured installation is deployed
- **WHEN** deployment finds the old composed entry with quota options
- **THEN** it removes old managed entries and artifacts, writes the four standalone entries once, and attaches the options only to quota
